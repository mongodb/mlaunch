...
import psutil
from mkit.util import OrderedDict
from mkit.util.cmdlinetool import BaseCmdLineTool
from mkit.util.print_table import print_table
from mkit.util.version import __version__
...
class MongoConnection(Connection):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs['appName'] = f'''mkit v{__version__}'''
...
class MKitTool(BaseCmdLineTool):
    ...
    def run(self, arguments=None):
        ...
        self.argparser.add_argument('--version', action='version',
                                    version=f'''mkit version {__version__} || Python {sys.version}''')
        ...
        self.argparser._action_groups[0].description = \
            ('init is the default command and can be omitted. To get help on '
             'individual commands, run mkit <command> --help. Command line '
             'arguments which are not handled by mkit will be passed '
    ...
    def init(self):
        ...
        # 'mkit init' if they are identical.
        ...
        if self.args['verbose']:
            print("writing .mkit_startup file.")
        ...
        if not all(map(itemgetter(1), ports_avail)):
            ...
            errmsg += (" * If you want to restart nodes from this "
                       "environment, use 'mkit start%s' instead.\n"
                       % dir_addon)
            errmsg += (" * If the ports are used by a different mkit "
                       "environment, stop those first with 'mkit stop "
                       "--dir <env>'.\n")
    ...
    def discover(self):
        ...
        # load .mkit_startup file for start, stop, list, use current
        # parameters for init
        if self.args['command'] == 'init':
            ...
            if not self._load_parameters():
                startup_file = os.path.join(self.dir, ".mkit_startup")
                raise SystemExit("Can't read %s, use 'mkit init ...' first."
                                 % startup_file)
    ...
    def _load_parameters(self):
        """
        Load the .mkit_startup file that exists in each datadir.

        Handles different protocol versions.
        """
        datapath = self.dir

        startup_file = os.path.join(datapath, '.mkit_startup')
    ...
    def _store_parameters(self):
        """Store startup params and config in datadir/.mkit_startup."""
        datapath = self.dir

        out_dict = {
            'protocol_version': 2,
            'mkit_version': __version__,
