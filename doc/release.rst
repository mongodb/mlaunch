...
The versioning standard in mongokit is pretty straight-forward. It follows a
major.minor.micro version system. We decided to start mongokit at version 1.0.0
...
#. Increase the version in ``./mkit/util/version.py`` from ``x.y.z-dev0`` to
   ``x.y.z``.
...
#. Bump the version on the develop branch (in ``./mkit/util/version.py``) to
   ``x.y.(z+1)-dev0``.
